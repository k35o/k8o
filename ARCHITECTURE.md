# アーキテクチャ

このドキュメントは、k8oプロジェクトのシステムアーキテクチャを説明します。

## 目次

- [システム概要](#システム概要)
- [技術スタック](#技術スタック)
- [モノレポ構成](#モノレポ構成)
- [アプリケーション構造](#アプリケーション構造)
- [データフロー](#データフロー)
- [デプロイメントアーキテクチャ](#デプロイメントアーキテクチャ)
- [設計思想](#設計思想)

## システム概要

k8oは、Next.js App Routerをベースとした個人プロジェクトポートフォリオサイトです。
ブログ、トーク、サービス紹介、インタラクティブなツール群を提供しています。

### 主な機能

- 📝 **ブログ** - MDXベースの技術ブログ（数式、シンタックスハイライト対応）
- 🎤 **トーク** - 登壇情報の管理
- 🛠️ **ツール** - 開発者向けユーティリティツール集
  - QRコード生成器
  - 色変換ツール
  - 文字数カウンター
  - Base変換器
  - など
- 🏷️ **タグシステム** - コンテンツのカテゴライズ
- 💬 **コメント** - ブログ記事へのコメント機能
- 📬 **メール購読** - 週次通知システム

## 技術スタック

### フロントエンド

| カテゴリ | 技術 | 用途 |
|---------|------|------|
| フレームワーク | Next.js | App Router、SSR/SSG |
| UIライブラリ | React | UI構築 |
| 言語 | TypeScript | 型安全性 |
| スタイリング | TailwindCSS | カスタムデザインシステム |
| コンポーネント | @k8o/arte-odyssey | 再利用可能UIコンポーネント |
| アニメーション | Motion | インタラクション |
| MDX | @next/mdx | ブログコンテンツ |
| 数式 | rehype-katex | 数式レンダリング |
| シンタックスハイライト | @shikijs/rehype | コードハイライト |

### バックエンド

| カテゴリ | 技術 | 用途 |
|---------|------|------|
| ORM | Drizzle ORM | データベース操作 |
| データベース | Turso (libSQL/SQLite) | メインデータストア |
| バリデーション | Zod | スキーマ検証 |
| メール | Resend | トランザクションメール |
| CMS | MicroCMS | ニュース管理 |

### 開発ツール

| カテゴリ | 技術 | 用途 |
|---------|------|------|
| モノレポ | Turbo | ビルドシステム |
| テスト | Vitest | ユニットテスト |
| E2E | Playwright | エンドツーエンドテスト |
| Storybook | Storybook 10 | コンポーネント開発 |
| リンター | Biome | コード品質 |
| Git hooks | Lefthook | pre-commit/pre-push |
| モック | MSW | APIモック |

## モノレポ構成

```
k8o/
├── apps/
│   └── main/                # メインNext.jsアプリケーション
│       ├── src/
│       │   ├── app/        # Next.js App Router
│       │   ├── services/   # ビジネスロジック
│       │   ├── emails/     # React Emailテンプレート
│       │   └── mocks/      # MSWモック定義
│       └── .storybook/     # Storybook設定
│
├── packages/
│   ├── database/           # データベースクライアントとスキーマ
│   │   ├── src/
│   │   │   ├── schema/    # Drizzleスキーマ定義
│   │   │   └── __mocks__/ # モックDB
│   │   └── migrations/    # マイグレーションファイル
│   │
│   ├── helpers/            # 共有ユーティリティ関数
│   │   └── src/
│   │       ├── array/     # 配列操作
│   │       ├── color/     # 色関連
│   │       ├── date/      # 日付操作
│   │       ├── number/    # 数値操作
│   │       ├── mdx/       # MDX処理
│   │       └── *.ts       # 個別ユーティリティ
│   │
│   └── typescript-config/  # 共有TypeScript設定
│
└── node_modules/           # 共有依存関係
```

### パッケージ依存関係

```
┌─────────────────────────────────────┐
│         @k8o/arte-odyssey           │ (外部npm)
│      UIコンポーネントライブラリ       │
└─────────────────────────────────────┘
                 ↑
                 │
┌────────────────┴────────────────────┐
│          apps/main                  │
│      Next.jsアプリケーション          │
└────────────────┬────────────────────┘
                 ↓
┌─────────────────────────────────────┐
│        @repo/database               │
│    データベースクライアント           │
└─────────────────────────────────────┘
                 ↓
┌─────────────────────────────────────┐
│        @repo/helpers                │
│      ユーティリティ関数               │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│     @repo/typescript-config         │ (共有設定)
│       TypeScript設定                │
└─────────────────────────────────────┘
```

## アプリケーション構造

### ディレクトリ構造の原則

```
apps/main/src/
├── app/                              # Next.js App Router
│   ├── _api/                         # アプリ共通API層
│   ├── _components/                  # アプリ共通コンポーネント
│   │   ├── global-layout/           # レイアウト
│   │   ├── link-card/               # リンクカード
│   │   └── ...
│   │
│   ├── blog/                         # ブログ機能
│   │   ├── (articles)/              # ルートグループ
│   │   │   └── [slug]/             # 動的ルート
│   │   ├── _api/                    # ブログ専用API
│   │   ├── _components/             # ブログ専用コンポーネント
│   │   └── page.tsx                # ブログ一覧ページ
│   │
│   ├── api/                         # APIルート
│   │   ├── blog/                   # ブログAPI
│   │   ├── subscriptions/          # 購読API
│   │   └── crons/                  # Cronジョブ
│   │
│   └── [tool-name]/                # 各ツール
│       ├── _components/            # ツール専用コンポーネント
│       ├── _utils/                 # ツール専用ユーティリティ
│       └── page.tsx               # ツールページ
│
├── services/                        # ビジネスロジック層
│   ├── blogs/                      # ブログサービス
│   ├── talks/                      # トークサービス
│   ├── tags/                       # タグサービス
│   ├── subscriptions/              # 購読サービス
│   └── email/                      # メール送信サービス
│
├── emails/                          # React Emailテンプレート
│   └── verification-email.tsx
│
└── mocks/                           # MSWモック定義
    ├── handlers/                   # モックハンドラー
    └── browser.ts                  # ブラウザ用MSW設定

packages/database/src/
├── schema/                          # Drizzleスキーマ定義
├── __mocks__/                       # モックDB
├── db.ts                            # DB接続
├── index.ts                         # エクスポート
└── utils.ts                         # DB操作ヘルパー
```

### レイヤーアーキテクチャ

```
┌─────────────────────────────────────┐
│         Presentation Layer          │
│      (app/, _components/)           │
│   - Next.js Pages/Layouts           │
│   - React Components                │
│   - Client/Server Components        │
└───────────────┬─────────────────────┘
                ↓
┌─────────────────────────────────────┐
│         Application Layer           │
│          (services/)                │
│   - Business Logic                  │
│   - Use Cases                       │
│   - External API Integration        │
└───────────────┬─────────────────────┘
                ↓
┌─────────────────────────────────────┐
│          Data Layer                 │
│         (database/)                 │
│   - Database Schema                 │
│   - ORM Operations                  │
│   - Data Access                     │
└─────────────────────────────────────┘
```

### Conditional Export Maps

環境に応じてモジュールを切り替える高度な機能：

```typescript
// packages/database/package.json
{
  "exports": {
    ".": {
      "types": "./src/index.ts",
      "storybook": "./src/__mocks__/db.ts",
      "node": "./src/index.ts",
      "default": "./src/index.ts"
    }
  }
}
```

**用途：**
- Storybook環境ではモックDBを使用
- 本番環境では実際のDBを使用

## データフロー

### ブログ記事表示のフロー

```
┌──────────────┐
│   Client     │ユーザーがブログページにアクセス
└──────┬───────┘
       ↓
┌──────────────┐
│ Next.js Page │ Server Component (SSG/ISR)
│  (RSC)       │
└──────┬───────┘
       ↓
┌──────────────┐
│   Services   │ getBlog(slug)
│  (blogs/)    │
└──────┬───────┘
       ↓
┌──────────────┐
│   Database   │ Drizzle ORM Query
│   (Turso)    │
└──────┬───────┘
       ↓
┌──────────────┐
│   Response   │ HTML with embedded data
└──────────────┘
```

### APIリクエストのフロー

```
┌──────────────┐
│   Client     │ POST /api/subscriptions/subscribe
└──────┬───────┘
       ↓
┌──────────────┐
│  API Route   │ Zod Validation
│ (route.ts)   │
└──────┬───────┘
       ↓
┌──────────────┐
│   Service    │ subscribe(email)
│ (subscriptions/)
└──────┬───────┘
       ↓
┌──────────────┐
│   Database   │ Insert subscriber
│   (Turso)    │
└──────┬───────┘
       ↓
┌──────────────┐
│    Email     │ Resend API
│   Service    │ Send verification
└──────┬───────┘
       ↓
┌──────────────┐
│   Response   │ { success: true }
└──────────────┘
```

## デプロイメントアーキテクチャ

### 本番環境

```
┌─────────────────────────────────────────────────────┐
│                    Internet                         │
└───────────────────────┬─────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│                  Vercel Edge Network                │
│              (CDN, Edge Functions)                  │
└───────────────────────┬─────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│              Next.js Application                    │
│              (Serverless Functions)                 │
└─────┬──────────────────────┬────────────────────────┘
      ↓                      ↓
┌─────────────┐      ┌──────────────┐
│    Turso    │      │  MicroCMS   │
│   (libSQL)  │      │    (CMS)    │
│  (Database) │      └──────────────┘
└─────────────┘              ↓
                      ┌──────────────┐
                      │   Resend     │
                      │   (Email)    │
                      └──────────────┘
```

### ローカル開発環境

```
┌─────────────────────────────────────────────────────┐
│              localhost:3000                         │
│           Next.js Dev Server                        │
└─────────────────────┬───────────────────────────────┘
                      ↓
┌─────────────────────────────────────────────────────┐
│              Turso Dev Server                       │
│            localhost:8787 (libSQL)                  │
└─────────────────────────────────────────────────────┘
```

開発環境の起動：
```bash
# ターミナル1: Tursoローカルサーバー
pnpm run -F @repo/database dev

# ターミナル2: Next.js開発サーバー
pnpm run dev
```

## 設計思想

### 1. Server-First Architecture

- デフォルトでServer Componentsを使用
- クライアントの負荷を最小化
- SEO最適化

```tsx
// Server Component (default)
export default async function BlogPage({ params }: Props) {
  const blog = await getBlog(params.slug); // サーバーで実行
  return <BlogLayout blog={blog} />;
}

// Client Component (必要な場合のみ)
'use client';
export function InteractiveButton() {
  const [count, setCount] = useState(0);
  return <button onClick={() => setCount(count + 1)}>{count}</button>;
}
```

### 2. Type Safety Everywhere

- Zodによるランタイムバリデーション
- Drizzle ORMによる型安全なDB操作
- TypeScript strict mode

```typescript
// スキーマ定義
const subscribeSchema = z.object({
  email: z.string().email(),
});

// 型推論
type SubscribeInput = z.infer<typeof subscribeSchema>;

// ランタイムバリデーション
const result = subscribeSchema.safeParse(data);
```

### 3. Separation of Concerns

- Presentation層（components）
- Application層（services）
- Data層（database）
- 各層の責務を明確に分離

### 4. Testing Strategy

- Helpers: In-source testing
- Components: Storybook stories
- Services: Vitest unit tests
- E2E: Playwright

### 5. Performance Optimization

- Incremental Static Regeneration (ISR)
- Edge Caching
- Image optimization (Next.js Image)
- Code splitting
- Bundle analysis (Sonda)

### 6. Accessibility First

- Semantic HTML
- ARIA attributes
- Keyboard navigation
- Storybook a11y addon
- axe-playwright for testing

### 7. Progressive Enhancement

- 基本機能はJavaScript無しでも動作
- クライアント機能は段階的に追加
- Graceful degradation

## パフォーマンス指標

### Core Web Vitals 目標

- LCP (Largest Contentful Paint): < 2.5s
- FID (First Input Delay): < 100ms
- CLS (Cumulative Layout Shift): < 0.1

### モニタリング

- Vercel Analytics
- Vercel Speed Insights
- Google Analytics

## セキュリティ

- CORS設定
- CSP (Content Security Policy)
- 環境変数の適切な管理
- SQL injection対策（Drizzle ORM）
- XSS対策（React自動エスケープ）

## スケーラビリティ

- Serverless architecture (Vercel)
- Edge functions for low latency
- Turso embedded replicas for low latency reads
- Static generation for content

---

詳細な実装ガイドラインは[CONTRIBUTING.md](./CONTRIBUTING.md)を参照してください。
