...
updatedAt: 2025-04-13 02:50:57+00
---
...
`wait`は指定したインデックスの値が指定した値である間待機します。待機中はスレッドはブロックされます。
...
`notify`は指定したインデックスの値で待機しているAgentに対して通知を送信します。
...
`Atomics.pause`には任意引数があり、待機時間についてのヒントを正の整数で渡します(ヒントが利用されることは保証されていません)。利用された場合は、与えられた値より小さい値を渡された時の待機時間以上は待つように動きます。
