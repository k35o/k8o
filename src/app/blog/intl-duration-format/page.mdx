...
# Intl.DurationFormatで期間を日本語で表現する
...
`style`には`long`と`short`と`narrow`と`digital`の4つがあります。それぞれ以下のような表示になります。
...
言い忘れましたが、期間は`years`、`months`、`weeks`、`days`、`hours`、`minutes`、`seconds`、`milliseconds`、`microseconds`、`nanoseconds`の9つに対応しています。これらを`format`に渡すオブジェクトのキーとして利用します。
