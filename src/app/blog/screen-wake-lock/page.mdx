...
これはタブが有効な範囲でのみScreen Wake Lock APIも有効になるようになっているためです。

先ほどのボタンは押すとロックを解除するためのボタンに切り替わっていましたが、タブを切り替えて戻ってくるとロックが解除されるのでロックを取得するためのボタンに戻ります。
...
`navigator.wakeLock.request`は第一引数にロックする画面についの種類を渡します。この引数は省略可能で、執筆現在は画面のスリープを制御する`screen`のみがサポートされています。
