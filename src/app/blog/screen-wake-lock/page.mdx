...
Screen Wake Lock APIは、デバイスの画面がスリープ状態に入るのを防ぐためのAPIです。料理中にレシピを閲覧してるユーザーへ向けてスリープしないようにするなど、処理を行わず継続的に画面を表示させたいアプリケーションに役立ちます。
...
先ほどのボタンはボタンを押すとロックを解除するためのボタンに切り替わっていましたが、タブを切り替えて戻ってくるとロックを取得するためのボタンに戻ります。

## Screen Wake Lock APIの使い方

Screen Wake Lock APIは、`navigator.wakeLock`を使用して取得します。

`navigator.wakeLock.request()`メソッドを使用すると、画面のスリープを防ぎ、`WakeLockSentinel`オブジェクトを返します。このオブジェクトは、スリープロックを解除するための`release()`メソッドを持っています。
...
`navigator.wakeLock.request`は第一引数に画面をロックする種類を渡します。この引数は省略可能で、執筆現在は画面のスリープを制御する`screen`のみがサポートされています。
...
2025年のBaseLineに追加された、Screen Wake Lock APIの紹介を行いました。ページを開いたままデバイスの外側で作業を行うときに悩まされるスリープの問題を解決してくれるより良い経験を提供してくれるAPIです。
