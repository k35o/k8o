...
description: Firefox 140でCSS Custom Highlight APIがサポートされ、DOMを変更せずに任意のテキスト範囲をハイライトできるようになりました。JavaScriptでRangeオブジェクトを定義し、Highlightオブジェクトとして登録後、CSSの::highlight擬似要素でスタイリングします。
...
これは、構文ハイライト、検索結果のハイライト、スペルチェックエラーの表示など、多くの用途で活用できる画期的な機能です。

## CSS Custom Highlight APIとは

`CSS Custom Highlight API`は、ページ内のDOMに影響を与えることなく任意のテキスト範囲をハイライトさせるAPIです。

ハイライトには`::selection`、`::spelling-error`、`::grammar-error`、`::target-text`のような擬似要素を拡張した`::highlight`擬似要素を利用します。
...
`HighlightRegistry`は`CSS.highlights`からアクセスできます。
...
`Highlight`オブジェクトは、一連の`Range`オブジェクトを同じハイライトのグループとして管理するためのものです。
...
そのため、今回は青色->黄色->赤色の順に登録したので、黄色が優先されて表示されています。

#### セマンティックな意味の指定

`type`プロパティはハイライトのセマンティックな意味を指定できます。これにより、支援技術がハイライトの内容を理解しやすくなります。
...
`highlight`には特別な意味がありません。`spelling-error`はスペルミスの強調、`grammar-error`は文法エラーの強調を意味します。
