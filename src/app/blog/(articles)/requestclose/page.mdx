...
description: HTMLDialogElementのrequestCloseメソッドは<dialog>要素を閉じるように要求します。「要求」するつまり、closeメソッドのように即座に閉じるのではなく、猶予を与えてから閉じます。閉じる処理を条件付きで制御することで、対話中の内容の保持が可能になり、より良いUXの提供が可能になります。
...
## HTMLDialogElement.requestClose Firefox
139で`HTMLDialogElement`の`requestClose`メソッドが実装され、Baseline
2025に仲間入りしました。
...
下の例は、ダイアログ内のフォームに入力中のテキストがあれば、「閉じる」でダイアログを閉じれないように実装しています。
