...
export const Autocomplete: FC<Props> = ({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'border-borderPrimary bg-bgBase relative w-full rounded-lg border shadow-xs',
        ...
        'has-aria-invalid:border-borderError',
        'has-disabled:border-borderDisabled has-disabled:has-[:hover]:hover:bg-bgBase has-disabled:cursor-not-allowed',
      )}
    >
      <div className="flex min-h-12 items-center justify-between gap-2 px-3 py-2">
        <div className="flex w-full flex-wrap gap-1">
          {value.map((text) => {
            ...
            return (
              <div
                key={text}
                tabIndex={-1}
                className="bg-bgSecondary inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm font-medium"
              >
                {label}
                <IconButton
                  label="閉じる"
          ...
          <input
            ...
            className={cn(
              'bg-bgTransparent grow focus-visible:outline-hidden',
      ...
      <div className="relative w-full">
        {open && (
          <div
            role="presentation"
            className="border-borderSecondary bg-bgBase absolute top-1 z-10 w-full rounded-lg border shadow-md"
          >
            <ul
              id={`${id}_listbox`}
              role="listbox"
            ...
            >
              {filteredOptions.length === 0 && (
                <li className="text-textDescription px-3 py-2">
                  該当なし
                </li>
              )}
