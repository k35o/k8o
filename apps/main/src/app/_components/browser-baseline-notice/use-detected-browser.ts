...
// dev 限定の確認用: ?baseline-notice=force を付けると、最新ブラウザでも
// 古い扱い(version 0)にして警告の見た目を実アプリ上で確認できる。本番では無効。
const FORCED_OUTDATED: DetectedBrowser = { browser: 'chrome', version: '0' };

export const isBaselineNoticeForced = (): boolean =>
  process.env['NODE_ENV'] === 'development' &&
  typeof window !== 'undefined' &&
  new URLSearchParams(window.location.search).get('baseline-notice') ===
    'force';

const getClientSnapshot = (): DetectedBrowser | null => {
  if (!detectionComputed) {
    detectedSnapshot = isBaselineNoticeForced()
      ? FORCED_OUTDATED
      : detectBrowser();
