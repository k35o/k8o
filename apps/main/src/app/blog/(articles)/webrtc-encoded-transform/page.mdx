---
title: WebRTC Encoded TransformでWebRTCの一連の処理にメスを入れる
description: WebRTC Encoded Transformは、WebRTCの送信・受信パイプラインにカスタム変換処理を挿入できる機能です。RTCRtpScriptTransformを使用してWorkerスレッドでエンコード済みフレームを直接操作し、E2EEやカスタムエフェクトを実装できます。
...
この記事では、WebRTC Encoded Transformの簡単な使い方を解説します。
