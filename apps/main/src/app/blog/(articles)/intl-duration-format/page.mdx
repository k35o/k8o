...
<BrowserSupportStatus featureId="intl-duration-format"></BrowserSupportStatus>

## はじめに

Firefox 136.0のリリースで`Intl`の`DurationFormat`がBaseline 2025に追加されました。
...
`locale`に応じた期間の出力は`Intl.DurationFormat`オブジェクトを作成して、`format`及びに`formatToParts`メソッドで行います。
...
## おわりに
Baseline 2025に追加された`Intl.DurationFormat`を紹介しました。

`locale`に基づいた期間の表現は、かなり実装が大変で、ライブラリを利用した場合でも満足が得られる結果を簡単に得ることはできませんでした。
そのため、`Intl.DurationFormat`によって、これらの表現を簡単にできるようになったことはとても嬉しいです。

`Intl`系の機能は日本・日本語であることによって、困っていた点を解決してくれるので今後の追加も楽しみです。
