...
updatedAt: 2026-08-25 11:57:00.000000+09
---
...
当初はこのPlaygroundで`revealOrder`や`tail`を切り替えながら`SuspenseList`を体験できましたが、その後のReact・Next.jsの更新で、このサイトが動く環境（React 19.2 / Next.js 16）では`SuspenseList`自体が利用できなくなりました。現在のPlaygroundでは、`SuspenseList`なしで複数の`Suspense`が並列に解決されていく様子と、フォールバックUIの有無による見え方の違いを確認できます。

<PlaygroundEmbed
  demo="SuspenseListによる読み込み制御"
  sectionId="suspense-list"
/>

`SuspenseList`が再び利用できるようになったら、`revealOrder`と`tail`を試せるPlaygroundに戻す予定です。
