...
description: details要素のコンテンツ部分にスタイルを適用できる::details-content擬似要素がBaseline 2025に加わりました。details要素を用いたスタイリングの守備範囲が広がっただけではなく、アニメーションの適用も容易になりました。
...
import { Playground, PlaygroundEmbed } from '@/app/_components/playgrounds';
...
<BrowserSupportStatus featureId="details-content" />
...
<PlaygroundEmbed demo="スタイリング例" sectionId="details-content" />
...
前者は`details`のすべての子要素にスタイルを割り当ててしまうのに対して、後者は常に`details`のコンテンツ部分にスタイルを適用できることがわかります。

ここから、`details`の子要素に依存せずに、`details`のコンテンツ部分にスタイルを付与できる`::details-content`擬似要素の強力さが読み取れます。
...
また、`::details-content`擬似要素は、開閉時のアニメーションを定義する際にも便利です。
...
<PlaygroundEmbed demo="アニメーション実装パターン" sectionId="details-content" />
