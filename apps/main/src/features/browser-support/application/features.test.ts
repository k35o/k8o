import {
  computeBrowserMinVersions,
  getReachedFeatures,
  getFeatureStatus,
  getBrowserSupportFeatures,
  selectReachedFeatures,
  selectFeatureStatus,
  selectBrowserSupportFeatures,
} from './features';
import type { BrowsersLike, FeaturesLike } from './features';
...
describe('selectReachedFeatures', () => {
  describe('正常系', () => {
    it('baseline(newly/widely)のみを日付降順で返す', () => {
      const result = selectReachedFeatures(featuresFixture);
  ...
  describe('異常系・エッジケース', () => {
    it('limited / moved / status 無し は含めない', () => {
      const ids = selectReachedFeatures(featuresFixture).map(
        (f) => f.featureId,
      );
...
describe('selectBrowserSupportFeatures', () => {
  describe('正常系', () => {
    it('baseline全部＋直近1年のlimitedを、直近で動いた日の降順で返す', () => {
      const result = selectBrowserSupportFeatures(
        featuresFixture,
        browsersFixture,
        NOW_MS,
      );
  ...
  describe('異常系・エッジケース', () => {
    it('1年より前にしか動いていない limited は除外する', () => {
      const ids = selectBrowserSupportFeatures(
        featuresFixture,
        browsersFixture,
        NOW_MS,
      ).map((f) => f.featureId);
...
describe('computeBrowserMinVersions', () => {
  describe('正常系', () => {
    it('baseline機能の対応版の、ブラウザごとの最大値を返す', () => {
      expect(computeBrowserMinVersions(featuresFixture)).toStrictEqual({
        chrome: '149',
        firefox: '146',
        safari: '26',
      });
  ...
  describe('異常系・エッジケース', () => {
    it('limited機能はフロア算出に含めない', () => {
      ...
      expect(computeBrowserMinVersions(featuresFixture).chrome).toBe('149');
    });
  });
});
...
describe('実データ(web-features)', () => {
  ...
  it('getReachedFeatures は newly/widely のみを日付降順で返す', () => {
    const features = getReachedFeatures();
  ...
  it('getBrowserSupportFeatures は baseline を包含し limited を追加する', () => {
    const baseline = getReachedFeatures();
    const platform = getBrowserSupportFeatures(NOW_MS);
