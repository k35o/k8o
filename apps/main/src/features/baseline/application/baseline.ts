import type { BrowserImplementationMap } from '@repo/helpers/browser/baseline-support';
...
export type SupportVersions = Partial<Record<BaselineBrowser, string>>;
...
export type PlatformStatus = 'widely' | 'newly' | 'limited';
...
): ((browser: BaselineBrowser, version: string) => string | null) => {
...
const toSupport = (
  support: SupportVersions,
  resolveDate: (browser: BaselineBrowser, version: string) => string | null,
): BrowserAvailability[] => {
...
export const selectBaselineFeatures = (
...
const toPlatformFeature = (
  ...
  return {
    featureId,
    name,
    status: toPlatformStatus(status.baseline),
    baselineDate,
...
export const selectPlatformFeatures = (
  featuresData: FeaturesLike,
  browsersData: BrowsersLike,
  nowMs: number,
): PlatformFeature[] => {
  ...
  const result: PlatformFeature[] = [];

  for (const resolved of resolveFeatures(featuresData)) {
    const feature = toPlatformFeature(resolved, resolveDate);
...
export const selectFeatureStatus = (
  ...
  return toPlatformFeature(resolved, makeReleaseDateResolver(browsersData));
};

export const computeBaselineMinVersions = (
  featuresData: FeaturesLike,
): BaselineMinVersions => {
...
export const getBaselineFeatures = (): BaselineFeature[] =>
  selectBaselineFeatures(features);

export const getPlatformFeatures = (nowMs: number): PlatformFeature[] =>
  selectPlatformFeatures(features, browsers, nowMs);

export const getFeatureStatus = (featureId: string): PlatformFeature | null =>
