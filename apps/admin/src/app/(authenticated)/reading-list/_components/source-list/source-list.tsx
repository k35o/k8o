...
export const SourceList: FC<{ sources: Source[] }> = ({ sources }) => {
  if (sources.length === 0) {
    return <EmptyState message="ソースがまだ登録されていません" />;
  }
  ...
  return (
    <Card appearance="shadow">
      <div className="flex flex-col">
        {sources.map((source) => (
          <Link
            className="border-border-mute hover:bg-bg-mute flex items-center gap-3 border-b px-5 py-4 text-sm transition-colors last:border-b-0"
