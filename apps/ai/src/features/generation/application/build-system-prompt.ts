...
export const buildSystemPrompt = (
  ...
  if (hasText(input.currentFile)) {
    sections.push(
      [
        '# 現在のファイル（これを土台に、ユーザー指示へ従って編集する）',
        '```tsx',
        input.currentFile,
        '```',
        '',
        '# 編集の出力形式（現在のファイルがあるときはこちらを厳守）',
        '- 変更が局所的（数カ所の修正・追加・削除）なら、全文ではなく ```edits フェンスで SEARCH/REPLACE ブロックだけを出力する:',
        '```edits',
        '<<<<<<< SEARCH',
        '（現在のファイルにある連続した行を、インデント・空行まで一字一句そのまま書く）',
        '=======',
        '（置き換え後の行）',
        '>>>>>>> REPLACE',
        '```',
        '- 変更箇所ごとにブロックを分け、ファイルの上から順に並べる。',
        '- SEARCH はその箇所を一意に特定できる最小の範囲にする（目安 3〜10 行。前後の文脈行を含めてよい）。',
        '- SEARCH が現在のファイルと一致しないと編集は失敗する。記憶や推測で書かず、必ず上記の現在のファイルからそのままコピーする。',
        '- レイアウトの大部分を作り直すような大きな変更のときだけ、```edits を使わず ```tsx で全文を出力する。',
        '- どちらの形式でも、最後に ```json フェンスの meta（出力フォーマットの 2.）を必ず出力する。',
      ].join('\n'),
    );
  }
  ...
  if (hasText(input.buildErrors)) {
    sections.push(
      [
        '# 前回の生成はビルド/実行時に次のエラーになった。原因を修正した全文を再生成すること（このターンは ```edits を使わず、必ず ```tsx の全文で出力する）',
        input.buildErrors,
      ].join('\n'),
    );
  }
